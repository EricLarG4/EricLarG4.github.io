...
Considering that each guanine is either in a syn or anti conformation (see section 1.7.1), there are 24 = 16 possible combinations per G-quartet. These will in turn yield 8 possible groove-widths combinations for a given core, interdependent with the loop geometries. There is no proper terminology for these groove-widths combinations yet. For monomolecular G-quadruplexes without snapbacks, the groove width can be inferred directly from the order of loop types (DOI: 10.1016/j.ymeth.2013.06.004).
...
Alternatively, it is possible to define 26 distinct (monomolecular) topologies based on glycosidic bond angles, using loops combination as a descriptor (DOI: 10.1016/j.ymeth.2013.06.004). This approach allows determining the width of grooves from the knowledge of loop geometries. Although these topologies are possible on paper, not all have been observed so far. On the other hand, the Z-G4 (left-handed topology) was not foreseen.
...
```{r fig5, echo=FALSE, fig.cap="Syn and anti glycosidic bond angles of guanosine (A), Stacking between two successive guanines in anti-anti (B; 2O3M, DOI: 10.1021/ja068739h), syn-anti (C; 2JPZ, DOI: 10.1093/nar/gkm522) and anti-syn (D; 1JPQ, DOI: 10.1016/S0022-2836(02)00428-X) conformations. The 5’-guanosine is on top (n) and the 3’ at the bottom (n+1).", out.width = '100%'}
